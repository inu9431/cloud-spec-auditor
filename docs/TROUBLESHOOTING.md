...
## #018: 파이프라인 부하 테스트 — bulk_create 처리량 및 쿼리 스케일링

**발생 시점:** 2026-03-30 (feature/pipeline-load-test, Docker Compose 로컬)

**목적:**
`UserInventory` 대량 적재 시 bulk_create 처리량과 인덱스 효과를 실측.

**테스트 환경:**
- Docker Compose 로컬 (db: PostgreSQL 16, web: Django 6.0)
- chunk_size=500 단위 bulk_create
- 3단계: 500행 / 5,000행 / 20,000행

**결과 — bulk_create 처리량**

| 규모 | 총 행수 | 적재 시간 | 처리량 |
|---|---|---|---|
| 10명 × 50개 | 500행 | 0.02s | 26,492 rows/s |
| 50명 × 100개 | 5,000행 | 0.17s | 29,266 rows/s |
| 100명 × 200개 | 20,000행 | 0.69s | 28,844 rows/s |

→ bulk_create 처리량은 규모와 무관하게 **~28,000 rows/s** 일정 (선형 확장)

**결과 — 쿼리 스케일링 (EXPLAIN ANALYZE)**

| 쿼리 | 500행 | 5,000행 | 20,000행 |
|---|---|---|---|
| 유저별 인벤토리 조회 (Index Scan) | 0.023ms | 0.016ms | 0.018ms |
| 전체 활성 인벤토리 수 (COUNT) | 0.066ms | 0.604ms | 2.577ms |
| provider별 집계 (GROUP BY) | 0.067ms | 0.592ms | 2.576ms |

→ 유저별 조회는 인덱스 덕분에 데이터 40배 증가에도 **0.02ms 수준 유지**
→ 전체 집계/GROUP BY는 Full Scan 불가피 — 데이터 비례 선형 증가

**발견된 병목:**
유저 생성(bcrypt 패스워드 해싱)이 실제 병목:
- 10명: 1s / 50명: 7s / 100명: 13s
- 인벤토리 적재(20,000행)는 0.69s로 오히려 유저 생성보다 빠름
- 실서비스에서는 회원가입이 단건이므로 파이프라인 운영상 문제 없음

**배운 점:**
bulk_create는 단건 insert 대비 극적으로 빠르며, chunk_size=500은 메모리/처리량 균형이 좋음.
인덱스가 걸린 컬럼 기준 필터 쿼리는 수만 행 규모에서도 0.02ms 수준 — 서비스 성능 여유 충분.
집계성 쿼리(COUNT *, GROUP BY)는 데이터 증가에 선형 비례하므로, 대규모 운영 시 별도 집계 테이블 또는 캐시 고려 필요.
