...
def test_compare_by_spec_region_filter(make_cloud_service):
    ...
    make_cloud_service(provider="AWS", instance_type="t3.medium", vcpu=2, memory_gb=Decimal("4"),
                       region_normalized="KR")
    make_cloud_service(provider="GCP", instance_type="n1-standard-1", vcpu=2, memory_gb=Decimal("4"),
                       region_normalized="JP")
    result = InstanceCompareService().compare_by_spec(vcpu=2, memory_gb=Decimal("4"), region_normalized="KR")
...
def test_reserved_pricing_excluded(make_cloud_service):
    ...
    make_cloud_service(provider="AWS", instance_type="t3.medium", vcpu=2, memory_gb=Decimal("4"), pricing_model=PricingModel.ON_DEMAND)
    make_cloud_service(provider="AWS", instance_type="t3.medium", vcpu=2, memory_gb=Decimal("4"), pricing_model=PricingModel.RESERVED, region="us-east-1")
...
def test_inactive_instance_excluded(make_cloud_service):
    ...
    make_cloud_service(provider="AWS", instance_type="t3.medium", vcpu=2, memory_gb=Decimal("4"), is_active=False)
