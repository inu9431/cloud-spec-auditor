...
def test_gcp_t2a_is_arm():
    ...
    dto = CloudServiceDTO.from_gcp(
        "t2a-standard-2", "asia-northeast3", "KR", 2, Decimal("8"), Decimal("0.07")
    )
    assert dto.cpu_arch == CpuArch.ARM64
...
def test_gcp_e2_micro_is_burstable():
    ...
    dto = CloudServiceDTO.from_gcp(
        "e2-micro", "asia-northeast3", "KR", 2, Decimal("1"), Decimal("0.01")
    )
    assert dto.is_burstable is True
...
def test_azure_b_series_is_burstable():
    ...
    dto = CloudServiceDTO.from_azure(
        {
