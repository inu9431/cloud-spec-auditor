...
class AzureSyncRequestSerializer(serializers.Serializer):
    region = serializers.CharField(default="koreacentral", help_text="Azure 리전명 (예: koreacentral)")


class AWSSyncRequestSerializer(serializers.Serializer):
    region = serializers.CharField(default="ap-northeast-2", help_text="AWS 리전명 (예: ap-northeast-2)")


class GCPSyncRequestSerializer(serializers.Serializer):
    region = serializers.CharField(default="asia-northeast3", help_text="GCP 리전명 (예: asia-northeast3)")
...
class InstanceCompareRequestSerializer(serializers.Serializer):
    ...
    memory_gb = serializers.DecimalField(required=False, max_digits=10, decimal_places=2, help_text="메모리 (GB)")
